...
// CommonJS, every file is module (by default)
// Modules - Encapsulated Code (only share minimum)
const names = require('./04-names') /*here the name of the variable can be xyz as it is an object but the content of the object should be `john` and `peter` as while exporting them only that was written only*/
